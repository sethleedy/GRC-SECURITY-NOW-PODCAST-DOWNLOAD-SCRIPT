...
EPISODE_NAME_VIDEO_HQ_URL=""
EPISODE_NAME_VIDEO_LQ_URL=""
...
function do_headers() {
	...
	#clear
	# Output title.
	echo "Seth Leedy's GRC Security Now Downloader v0.2"
...
if ! $pretend_mode ; then
	...
	for (( c=$EPISODE; c<=$EPISODE_TO; c=$((c+$par_downloads_count)) )); do
		...
		if [ $download_video_hq ]; then
			if ! $quite_mode ; then
				echo "Can't download until the video URL is found and entered."
