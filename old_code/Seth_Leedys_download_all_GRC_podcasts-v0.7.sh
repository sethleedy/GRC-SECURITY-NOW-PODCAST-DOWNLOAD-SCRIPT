...
function output_help() {
	...
	echo " "
	echo "Search Mode:"
	echo "-dandstxt	Download and Search, will download all text episodes and show notes, temporarily, by default and search for the text you enter here. -ep use optional"
	echo "OR"
	echo "-stxt		Search local directory .txt episodes and show notes for text you enter here. -ep use optional"
	echo " "
	...
	echo " "
	echo "-q	Quite mode. Nothing but errors will be outputted to the screen."
	...
	while [[ $c -le $EPISODE_TO ]]; do
		...
		if  $download_audio_hq ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_HQ"`
		...
		if  $download_audio_lq ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do # Does not like variable $d within here on the second expression in the loop. Have to use $c.
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_LQ"`
				ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
				pid[$d]=$ttpid
		...
		if  $download_episode_text ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				if ! $quite_mode || $search_echo_override_mode ; then
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_TEXT"`
				ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
				pid[$d]=$ttpid
		...
		if  $download_episode_pdf ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_PDF"`
				ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
				pid[$d]=$ttpid
		...
		if  $download_episode_html ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_HTML"`
				ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
				pid[$d]=$ttpid
		...
		if  $download_episode_shownote ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				tpid=`wget -N -c -qb "$EPISODE_NAME_AUDIO_SHOWNOTES"`
				ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
				pid[$d]=$ttpid
		...
		if $download_video_hq ; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				if [ $? -eq 0 ]; then
					...
					tpid=`wget -N -c -qb "$EPISODE_NAME_VIDEO_HQ"`
					ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
					pid[$d]=$ttpid
				else
					if ! $quite_mode ; then
		...
		if $download_video_lq; then
			for (( d=$c; d<$(($c+$slot_downloads)); d++ )); do
				...
				if [ $? -eq 0 ]; then
					...
					tpid=`wget -N -c -qb "$EPISODE_NAME_VIDEO_LQ"`
					ttpid=(`echo $tpid | cut -d " " -f 5 | cut -d "." -f 1`)
					pid[$d]=$ttpid
				else
					if ! $quite_mode ; then
...
function do_searching() {
	...
	if $search_txt_download ; then
		...
		cd ..
		#rm $download_temp_txt_search_dir/*
		#rmdir $download_temp_txt_search_dir
	...
	if $search_txt_local ; then
		grep -w -l -s -i "$search_string" *.txt > .found
